...
export default {
  ...
  methods: {
    ...
    // 비디오 불러오기
    getVideo() {
      const path = `${process.env.VUE_APP_VIDEO}${
        this.vodEpiInfo.episodeInfo.gd_id
      }_${this.vodEpiInfo.episodeInfo.v_title.replace(/(\s*)/g, '')}_${
        this.vodEpiInfo.episodeInfo.ve_episode_num
      }화`;
  ...
  beforeDestroy() {
    ...
    console.log(
      ...
      '->',
      this.nowTime(this.videoCurrentTime)
    );
  },
};
</script>
