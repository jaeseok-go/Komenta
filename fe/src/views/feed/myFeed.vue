<template>
    ...
    (피드로 들어가는법...? u_id??)
        <div>
        <!-- 유저 -->
        1. 유저 팔로잉 정보 GET -> 토큰에서 가져올 수 있음(u_nickname, u_profile_pic,)
        로그인한 유저의 정보가 필요함
        유저의 팔로워, 팔로잉 수
        <!-- 타유저 -->
        ...
        시청기록 테이블
        GET(u_id) -> 조회 get방식 전부 다 불러오기
        vh_id, vh_wathching_time, vh_score, ve_id, vh_good,vh_score->vh_name???
        ...
        플레이리스트 이름을 적고 확인누르면 POST요청 가고 플레이리스트를 저장하고 피드에 뜨게함
        2. 플레이리스트에 VOD추가
